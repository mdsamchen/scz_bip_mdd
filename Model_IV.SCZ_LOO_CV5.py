...
HOME_DIR = "D:/SCZ_BP_MD_PRS/SCZ/with_no_SCZ_PRS/"
os.chdir(HOME_DIR)
...
SCZ_all = pd.read_csv('SCZ_newIID_Diag_SEX_from_35traitsPRSbest_noSCZ_rescaled.csv', header=0)
...
SCZ_all_prs = np.reshape(SCZ_all_prs, (-1, 36, 1))
...
inputDim = 36
prsInput = Input(shape=(36, 1, ))

# do leave one out cross validation procedure
...
for i in range(cvFold):      
    ...
    prs_dnn = Dense(units=256, activation='relu')(prs_dnn)
    prs_dnn = Dense(units=256, activation='relu')(prs_dnn)
    ...
    pyplot.savefig('SCZ_newIID_Diag_SEX_35traitsPRSbest_CV5_v5_cv_' + str(cvFold) + '_cm_' + str(i) +'.png')
    pyplot.close()
    ...
    pyplot.xticks(fontsize=14)
    pyplot.yticks(fontsize=14)
    pyplot.savefig('SCZ_newIID_Diag_SEX_35traitsPRSbest_CV5_cv_' + str(cvFold) + '_acc_' + str(i) +'.png')
    ...
    pyplot.xticks(fontsize=14)
    pyplot.yticks(fontsize=14)
    pyplot.savefig('SCZ_newIID_Diag_SEX_35traitsPRSbest_CV5_cv_' + str(cvFold) + '_ROC_' + str(cvFold + i) + '.png')
    i = i + 1
